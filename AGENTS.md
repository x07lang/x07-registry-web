...
- `scripts/check_generated_spec_mirror.sh` auto-discovers sibling `../x07` and `../x07-wasm-backend` checkouts before falling back to `_deps/`, so run it from a normal `x07lang/` workspace before commit.
